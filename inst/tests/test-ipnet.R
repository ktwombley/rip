context("ipnet OO basics")

test_that("ipnet() returns an ipnet object", {
  ipno <- ipnet(1L)

  expect_that(ipno, is_a("ipnet"))  
  
})

test_that("is.ipnet() works", {
  ipno <- ipnet(0L)  #That's a zero

  expect_that(is.ipnet(ipno), is_true())
  expect_that(is.ipnet("foo"), is_false())
})

test_that("ipnet() understands numeric input", {
  ipno <- ipnet(2130706433L)
  expect_that(as.integer(ipno), equals(2130706433L))
  expect_that(as.numeric(ipno), equals(2130706433L))
  expect_that(as.double(ipno), equals(2130706433L))
  expect_that(as.character(ipno), equals("127.0.0.1/32"))
    
})

test_that("ipnet(numeric x) internal structure is correct", {
  ipno <- ipnet(185273099)
  expect_that(ipno$address, is_a("ip"), "address vector is not an ip")
})

test_that("ipnet() understands ip character strings", {

  tmp <- throws_error()(ipnet("127.0.0.1"))
  expect_that(tmp$passed, is_false())

  tmp <- gives_warning()(ipnet("127.0.0.1"))
  expect_that(tmp$passed, is_false(), "ipnet(\"127.0.0.1\") emitted a warning.")

  ipno <- ipnet("1:2:3:4:5:6:7:8")
  expect_that(ipno$af, equals(AF_INET6), "ipv6 sets correct AF") 
  expect_that(ipno$address, equals(ip("1:2:3:4:5:6:7:8")), "ipv6 set incorrect address")

  ipno <- ipnet("127.0.0.1")
  expect_that(ipno$af, equals(AF_INET), "ipv4 sets correct AF") 
  expect_that(ipno$address, equals(
  ipno <- ipnet("127.0.0.1")
), "ipv4 set incorrect address")
